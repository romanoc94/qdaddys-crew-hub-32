-- First, let's check the current constraint on training_templates
SELECT conname, pg_get_constraintdef(oid) 
FROM pg_constraint 
WHERE conname LIKE '%level%' AND conrelid = 'training_templates'::regclass;

-- Drop the existing constraint if it exists
ALTER TABLE training_templates DROP CONSTRAINT IF EXISTS training_templates_level_check;

-- Add a new constraint that allows the correct values
...
-- Add comprehensive template tasks for all 8 templates
