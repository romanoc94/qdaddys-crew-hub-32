...
INSERT INTO public.checklist_templates (store_id, name, description, checklist_type, is_active, created_by) 
SELECT 
  s.id, 
  'Opening Checklist', 
...
JOIN public.profiles p ON p.store_id = s.id AND p.role = 'operator'
WHERE NOT EXISTS (
...
INSERT INTO public.checklists (template_id, store_id, date, status, created_by)
SELECT 
  ct.id,
  ct.store_id,
  CURRENT_DATE,
  ...
  p.id
FROM public.checklist_templates ct
JOIN public.profiles p ON p.store_id = ct.store_id AND p.role = 'operator'
WHERE NOT EXISTS (
