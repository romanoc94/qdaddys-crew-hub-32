...
BEGIN
    ...
    IF sample_store_id IS NOT NULL THEN
        -- Insert opening checklist template (with conflict check)
        ...
        IF template_opening_id IS NULL THEN
            INSERT INTO public.checklist_templates (name, description, checklist_type, store_id, is_active)
            VALUES ('Daily Opening Checklist', 'Essential tasks to open the BBQ restaurant', 'opening', sample_store_id, true)
        ...
        -- Insert closing checklist template (with conflict check)
        ...
        IF template_closing_id IS NULL THEN
            INSERT INTO public.checklist_templates (name, description, checklist_type, store_id, is_active)
            VALUES ('Daily Closing Checklist', 'End of day cleanup and shutdown procedures', 'closing', sample_store_id, true)
        ...
        -- Insert sample shifts for today (checking for existence first)
        IF NOT EXISTS (SELECT 1 FROM public.shifts WHERE store_id = sample_store_id AND date = CURRENT_DATE AND shift_type = 'opening') THEN
            INSERT INTO public.shifts (store_id, date, shift_type, start_time, end_time, notes, daily_specials, catering_notes)
            VALUES (sample_store_id, CURRENT_DATE, 'opening', '06:00', '14:00', 'Morning prep and lunch rush', 'Smoked brisket special with cornbread', 'Corporate lunch order for 50 people at 12:30');
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM public.shifts WHERE store_id = sample_store_id AND date = CURRENT_DATE AND shift_type = 'dinner') THEN
            INSERT INTO public.shifts (store_id, date, shift_type, start_time, end_time, notes, daily_specials, catering_notes)
            VALUES (sample_store_id, CURRENT_DATE, 'dinner', '14:00', '22:00', 'Afternoon and dinner service', 'Ribs and pulled pork combo deals', 'Birthday party setup for 8 people at 6 PM');
        END IF;
        
        -- Create sample checklists for today
        IF NOT EXISTS (SELECT 1 FROM public.checklists WHERE template_id = template_opening_id AND date = CURRENT_DATE) THEN
            INSERT INTO public.checklists (template_id, store_id, date, status)
            VALUES (template_opening_id, sample_store_id, CURRENT_DATE, 'pending');
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM public.checklists WHERE template_id = template_closing_id AND date = CURRENT_DATE) THEN
            INSERT INTO public.checklists (template_id, store_id, date, status)
            VALUES (template_closing_id, sample_store_id, CURRENT_DATE, 'pending');
        END IF;
